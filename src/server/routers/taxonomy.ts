...
export const taxonomyRouter = router({
  ...
  createDimension: withPermission("knowledge", "write").input(z.object({ number: z.number(), name: z.string(), layer: z.string(), tenantId: z.string() })).mutation(({ ctx, input }) => ctx.prisma.dimensionConfig.create({ data: input })),
  updateDimension: withPermission("knowledge", "write").input(z.object({ id: z.string(), number: z.number().optional(), name: z.string().optional(), layer: z.string().optional() })).mutation(({ ctx, input }) => ctx.prisma.dimensionConfig.update({ where: { id: input.id }, data: input })),
