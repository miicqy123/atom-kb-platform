...
const MOCK_RUN_TREND = Array.from({ length: 14 }, (_, i) => ({
  ...
  success: 25 + Math.floor(Math.random() * 30),
}));
...
export default function GovernanceDashboard() {
  ...
  const successRate = totalRuns > 0
    ? ((runStats.find((s: any) => s.status === "SUCCESS")?._count || 0) / totalRuns * 100).toFixed(1) + "%"
    : "0%";
      ...
      <div className="mb-4 flex gap-1">
        {TABS.map((t, i) => (
          <button
            key={t}
            onClick={() => setTab(i)}
            className={`rounded-lg px-4 py-2 text-sm ${tab === i ? "bg-brand text-white" : "border hover:bg-gray-50"}`}
          >
            {[
              <Activity key={0} className="inline h-4 w-4 mr-1" />,
              <Shield key={1} className="inline h-4 w-4 mr-1" />,
              <Coins key={2} className="inline h-4 w-4 mr-1" />,
              <AlertTriangle key={3} className="inline h-4 w-4 mr-1" />,
            ][i]}
            {t}
          </button>
        ))}
      ...
      {tab === 1 && (
        <div className="space-y-6">
          ...
          <div className="rounded-xl border bg-white p-4">
            ...
            <ResponsiveContainer width="100%" height={280}>
              <LineChart
                data={qualityData?.slice(0, 14).map((d: any, i: number) => ({
                  day: `第${i + 1}次`,
                  s8: d.passed ? 100 : 0,
                  s9: d.s9OverallScore,
                })) || []}
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-3 gap-4">
            ...
            <StatsCard
              title="日均 Token"
              value={
                (projectDashboardData?.workflowStats as any[])?.reduce(
                  (sum: number, stat: any) => sum + (stat._avg?.tokenUsage || 0),
                  0
                )?.toFixed(0) || "0"
              }
              color="text-pf"
        ...
        <div className="rounded-xl border bg-white p-4">
          ...
          <div className="space-y-2">
            {(incidents ?? []).map((inc: any) => (
              <div key={inc.id} className="flex items-center justify-between rounded-lg border p-3">
                <div className="flex items-center gap-3">
                  <span className={`rounded px-2 py-0.5 text-xs font-medium ${inc.severity === "CRITICAL" ? "bg-red-100 text-red-700" : inc.severity === "HIGH" ? "bg-orange-100 text-orange-700" : "bg-yellow-100 text-yellow-700"}`}>
                    {inc.severity}
                  </span>
                  <div>
                    <p className="text-sm">{inc.type}</p>
                    <p className="text-xs text-gray-400">{new Date(inc.createdAt).toLocaleString("zh-CN")}</p>
                  </div>
                </div>
            ...
            {(!incidents || incidents.length === 0) && (
              <p className="text-sm text-gray-400 text-center py-8">暂无异常事件</p>
            )}
          </div>
        </div>
      )}
