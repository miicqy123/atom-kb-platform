...
function WorkbenchContent() {
  ...
  const markdownPreview = rawData?.markdownContent || `# 品牌话术手册
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 py-3 border-b bg-white flex items-center gap-4">
        ...
        <div>
          <h1 className="text-base font-bold">📄 {rawData?.title || "品牌话术手册.pdf"}</h1>
          <div className="flex items-center gap-2 text-xs text-gray-500">
            <StatusBadge status="CONVERTED" />
            <span>材料类型: {rawData?.materialType || "话术库"}</span>
            <span>经验源: {rawData?.experienceSource || "E1"}</span>
          </div>
        </div>
