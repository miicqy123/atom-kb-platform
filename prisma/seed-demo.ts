...
async function main() {
  ...
  // 清理旧的演示数据（如果存在）
  await prisma.reviewTask.deleteMany({ where: { workflowRun: { projectId } } });
  await prisma.evaluationRecord.deleteMany({ where: { workflowRun: { projectId } } });
  await prisma.workflowRun.deleteMany({ where: { projectId } });
  await prisma.incident.deleteMany({ where: { workflowRun: { projectId: projectId } } });
  await prisma.agentBlueprint.deleteMany({ where: { agent: { projectId } } });
  await prisma.agent.deleteMany({ where: { projectId } });
  await prisma.atomBlueprint.deleteMany({ where: { blueprint: { projectId } } });
  await prisma.blueprint.deleteMany({ where: { projectId } });
  await prisma.basePack.deleteMany({ where: { scopeValue: projectId } });
  await prisma.basePack.deleteMany({ where: { scope: 'global' } });
  await prisma.atom.deleteMany({ where: { projectId } });
  await prisma.qaPair.deleteMany({ where: { projectId } });
  await prisma.raw.deleteMany({ where: { projectId } });
  await prisma.dimensionConfig.deleteMany();
  await prisma.auditLog.deleteMany({ where: { user: { tenantId: 'default-tenant' } } });
  ...
  const atomData = [
    // A层（公司认知）
    { title: '公司简介：金钥匙家装成立于2015年', content: '金钥匙家装是一家专注于中高端全屋定制的装饰公司，成立于2015年。公司总部位于上海，目前已在北京、广州、深圳设立分公司。我们秉承“匠心品质，贴心服务”的理念，致力于为客户打造理想的家居生活空间。经过8年的发展，我们服务了超过5000户家庭，客户满意度达到96%以上。公司拥有一支50余人的专业设计师团队和标准化施工队伍，确保每一个项目都能高质量完成。', layer: 'A', granularity: 'ATOM', slotMappings: ['S0'], dimensions: [1, 2], status: 'ACTIVE' },
    { title: '品牌定位：中高端全屋定制', content: '金钥匙家装定位于中高端市场，专注于全屋定制解决方案。我们的服务对象主要是对生活品质有较高要求的中产家庭，特别是对设计感、环保性和实用性有明确需求的客户群体。相比低端装修公司，我们更注重设计创新和工艺品质；相比传统高端装修公司，我们提供更具性价比的解决方案。公司采用直营管理模式，确保服务质量的一致性和可控性。', layer: 'A', granularity: 'ATOM', slotMappings: ['S1'], dimensions: [1, 3], status: 'ACTIVE' },
    { title: '核心优势：设计师一对一服务', content: '金钥匙家装的核心竞争优势在于提供专业设计师一对一的全程跟踪服务。每位客户都会配备专属设计师，从初步沟通、现场勘测、方案设计、材料选择、施工监督到最终验收，设计师全程参与。这种服务模式确保了设计理念的一致性和实施的准确性，同时也提升了客户的满意度和信任度。设计师团队均具有5年以上从业经验，定期接受专业培训和客户沟通技巧训练。', layer: 'A', granularity: 'ATOM', slotMappings: ['S0'], dimensions: [2, 4], status: 'ACTIVE' },
    { title: '服务范围：覆盖全屋设计、施工、软装', content: '金钥匙家装提供全屋一体化服务，包括：1.室内设计：户型分析、风格定位、功能规划、3D效果图；2.基础施工：水电改造、泥瓦工程、木工制作、油漆涂装；3.主材供应：地板瓷砖、洁具厨具、门窗灯具等；4.软装配饰：窗帘布艺、装饰摆件、绿植花卉等；5.环保检测：甲醛检测、空气净化、治理建议。客户只需一个电话，即可享受从毛坯到拎包入住的全套服务。', layer: 'A', granularity: 'MODULE', slotMappings: ['S1'], dimensions: [3, 5], status: 'ACTIVE' },
    { title: '明星案例：万科城市花园整装项目', content: '万科城市花园整装项目是金钥匙家装的标杆案例，面积120平米，四室两厅格局。业主为年轻夫妇，希望打造简约现代风格的温馨居所。设计亮点：客厅开放式布局，主卧套房设计，儿童房多功能规划，书房榻榻米设计。项目周期：设计15天，施工90天。客户反馈：对设计方案和施工质量高度满意，已推荐3位朋友使用我们的服务。该项目荣获2023年上海装饰设计奖优秀奖。', layer: 'A', granularity: 'ATOM', slotMappings: ['S2'], dimensions: [6, 7], status: 'DRAFT' },
    { title: '客户画像：25-45岁中产家庭', content: '金钥匙家装的目标客户群体主要是年龄在25-45岁之间的中产阶层家庭，主要包括：1.新婚夫妇：首次置业装修，追求实用性和美观性的平衡；2.二孩家庭：对环保性和安全性有更高要求；3.改善型客户：对设计品质和个性化有更高期待；4.投资型客户：关注保值增值和出租收益。这类客户群体具有较强的消费能力、对品质有一定要求、重视服务体验、决策相对理性但也有感性因素。', layer: 'A', granularity: 'ATOM', slotMappings: ['S3'], dimensions: [8, 9], status: 'ACTIVE' },
    { title: '价格体系：688/888/1288三档套餐', content: '金钥匙家装推出标准化套餐服务，方便客户选择：1.标准套餐688元/平米：适用于100平米以下中小户型，包含基础设计和施工，主要材料品牌为国产品牌，工期60天；2.精品套餐888元/平米：适用于中等户型，增加部分进口材料，设计更为精致，工期75天；3.豪华套餐1288元/平米：适用于大户型和别墅，进口高端材料，个性化设计，顶级工艺，工期90天。所有套餐均包含水电改造、基础施工、主材供应，不含家具家电。', layer: 'A', granularity: 'ATOM', slotMappings: ['S2'], dimensions: [4, 10], status: 'TESTING' },
    { title: '售后承诺：质保5年，24小时响应', content: '金钥匙家装对所有工程项目提供完善的售后服务承诺：1.质量保证：主体结构质保10年，水电隐蔽工程质保8年，其他装修部分质保5年；2.服务响应：客户问题反馈后24小时内响应，48小时内给出解决方案；3.维修服务：质保期内免费维修，质保期外收取材料成本费；4.定期回访：完工后每季度回访一次，每年提供一次免费保养服务；5.客户权益：享有7天无理由退订、15天设计不满意重做、终身咨询服务等权益。', layer: 'A', granularity: 'ATOM', slotMappings: ['S7'], dimensions: [5], status: 'ACTIVE' },

    // B层（业务技能）
    { title: '量房流程：预约→上门→测量→出图', content: '标准量房服务流程：1.预约确认：提前1天确认上门时间，携带量房工具和测量软件；2.现场沟通：了解客户需求和重点关注区域，拍摄现场照片；3.精确测量：使用激光测距仪测量尺寸，绘制平面图，记录梁柱位置、管线走向；4.细节记录：记录地面平整度、墙体垂直度、采光情况、通风状况；5.数据整理：现场录入测量数据，生成CAD平面图；6.后续安排：约定方案沟通时间，发送量房报告。全程预计2小时，确保数据准确无误。', layer: 'B', granularity: 'ATOM', slotMappings: ['S5'], dimensions: [11, 12], status: 'ACTIVE' },
    { title: '报价技巧：先了解预算再推方案', content: '有效报价技巧包括：1.预算探询：通过聊天了解客户大致预算范围，避免推荐超出预算的方案；2.价值先行：先展示设计价值和工艺优势，再提及价格；3.套餐推荐：根据预算推荐合适套餐，突出性价比；4.费用分解：将总价分解为设计费、材料费、施工费等明细；5.优惠策略：适时提及活动优惠或老客户推荐折扣；6.支付方式：介绍灵活的支付方式和分期选项。记住：价格是结果，不是起点。', layer: 'B', granularity: 'ATOM', slotMappings: ['S5'], dimensions: [13, 14], status: 'ACTIVE' },
    { title: '异议处理：太贵了→价值对比法', content: '面对“太贵了”异议的处理方法：1.认同感受：“我理解您对价格的关注，这是明智的决定”；2.价值强调：“相比传统装修公司，我们提供的是设计师一对一定制服务”；3.对比分析：列出同类服务市场价格区间，突显我们的性价比；4.细化说明：将总费用分解为每日投入成本，降低心理负担；5.案例佐证：分享其他客户满意度反馈；6.限时优惠：适当时候可以提及当前活动。核心：让客户觉得物有所值。', layer: 'B', granularity: 'ATOM', slotMappings: ['S5'], dimensions: [15, 16], status: 'TESTING' },
    { title: '签单流程：意向确认→合同签署→定金', content: '标准化签单流程：1.意向确认：客户对方案和价格无异议后，确认签约意向；2.合同准备：准备正式合同文本，包括设计、施工、材料等详细条款；3.合同讲解：逐条解释合同条款，确保客户理解各项内容；4.合同签署：双方签字盖章，留存副本；5.定金收取：收取合同金额20%的定金，开具收据；6.开工安排：确定开工日期，安排项目经理和施工队。全程需耐心细致，确保客户充分知情。', layer: 'B', granularity: 'ATOM', slotMappings: ['S5'], dimensions: [17], status: 'ACTIVE' },
    { title: '售后处理：投诉分级→24h响应→48h方案', content: '售后投诉处理流程：1.投诉登记：详细记录投诉内容、客户信息、期望解决方式；2.等级划分：一级（一般问题）、二级（重要问题）、三级（紧急问题）；3.响应时限：一级24小时、二级12小时、三级2小时内响应；4.方案制定：分析问题原因，制定解决方案；5.执行跟进：落实解决方案，持续跟进客户满意度；6.总结改进：问题解决后总结经验，完善预防措施。', layer: 'B', granularity: 'ATOM', slotMappings: ['S5'], dimensions: [18, 19], status: 'DRAFT' },
    { title: '转介绍话术：老客户推荐返现2000元', content: '客户转介绍激励话术：“如果您身边有朋友需要装修服务，欢迎推荐给我们。为了感谢您的信任，我们对成功推荐的老客户给予2000元现金奖励。推荐流程很简单：1.联系我们告知推荐人信息；2.我们安排专人跟进；3.签约完成后，奖励直接发放给您。这不仅帮助朋友找到可靠的装修服务，也让您获得实实在在的好处。”注意：所有推荐需客户知情同意。', layer: 'B', granularity: 'ATOM', slotMappings: ['S6'], dimensions: [20], status: 'ACTIVE' },

    // C层（风格红线）
    { title: '禁止承诺：不得承诺具体工期', content: '销售和服务人员严禁向客户承诺具体完工日期，只能提供合理的时间范围。因为装修过程中存在诸多不确定因素：天气影响、材料延迟、客户需求变更、设计调整等。正确的表述方式：“我们计划在XX天内完成，具体时间将根据施工进展和天气情况动态调整，我们会及时向您汇报进度”。如需特殊工期要求，需单独评估并签署补充协议。违反此规定的员工将面临警告直至解雇处分。', layer: 'C', granularity: 'ATOM', slotMappings: ['S4'], dimensions: [21, 22], status: 'ACTIVE' },
    { title: '价格红线：严禁私下打折超过95折', content: '所有价格优惠政策必须通过公司系统申请和审批，销售人员严禁私下给予客户超过95折的价格优惠。特殊情况（如老客户介绍、大额订单等）需经部门经理审批，并在系统中备案。私自打折的行为严重损害公司利益，一经发现将面临严重处罚，包括但不限于扣减提成、降职甚至解雇。公司有完善的促销政策和优惠体系，所有优惠必须公开透明，确保公平竞争。', layer: 'C', granularity: 'ATOM', slotMappings: ['S4'], dimensions: [23], status: 'ACTIVE' },
    { title: '话术规范：不得贬低竞品', content: '在与客户交流过程中，严禁贬低或攻击竞争对手。当客户提及竞品时，应采取客观对比的方式，重点突出我们产品的优势，而不是贬低对方的劣势。正确的做法：“每家公司都有自己的特色，我们专注于提供专业的设计服务和精细的施工管理，您可以综合比较后再做决定”。这样既体现了我们的专业素养，也避免了恶意竞争的风险。我们靠实力和服务赢得客户，不是靠攻击对手。', layer: 'C', granularity: 'ATOM', slotMappings: ['S4'], dimensions: [24, 25], status: 'ACTIVE' },

    // D层（系统合规）
    { title: '数据合规：客户信息不得外传', content: '根据《个人信息保护法》等相关法规，所有客户信息属于高度保密数据，任何人不得私自外传、泄露或挪作他用。1.信息访问：仅限工作需要的员工访问客户信息；2.信息传输：使用加密通道传输，禁止通过微信、QQ等非加密方式发送；3.信息存储：客户信息必须存储在公司指定的安全系统中；4.信息销毁：过期信息按规定流程销毁；5.违规处罚：泄露客户信息者将承担法律责任并立即解雇。保护客户隐私是我们的基本义务。', layer: 'D', granularity: 'ATOM', slotMappings: ['S4'], dimensions: [26, 27], status: 'ACTIVE' },
    { title: '法律条款：合同必须包含七天冷静期', content: '根据相关法律法规，家装合同中必须包含“七天冷静期”条款，即客户在合同签订后的七天内有权无理由解除合同，公司应全额退还已收款（如有设计费等特殊约定除外）。此条款必须在合同中明确标注，销售人员在签约时必须主动告知客户此项权利。冷静期内解约，不得收取违约金或手续费。此举既保护消费者权益，也体现我们对自身服务的信心。所有销售人员必须熟知此条款。', layer: 'D', granularity: 'ATOM', slotMappings: ['S8'], dimensions: [28, 29], status: 'ACTIVE' },
    { title: '审计要求：每次报价必须系统留痕', content: '为了加强财务审计和流程管控，每一次客户报价都必须在公司系统中留有完整记录。1.报价录入：销售人员必须将详细报价单录入系统；2.版本管理：每次报价调整都需保留历史版本；3.审批流程：大额报价需经过上级审批；4.变更记录：客户要求的任何变更都需系统备注；5.归档保存：报价记录永久保存备查。系统自动审计，无系统记录的报价视为无效。这是合规经营的基本要求。', layer: 'D', granularity: 'ATOM', slotMappings: ['S9'], dimensions: [30], status: 'ACTIVE' },
  ];
